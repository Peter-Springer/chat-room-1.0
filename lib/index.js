...
function renderMessages() {
  ...
  for (i= chatroom.messages.length - 10; i <= chatroom.messages.length - 1; i++){
...
function createTemplate(id, username, input) {
  $('.all-messages').append(
    `<li data=${username} id=${id}>
      <p class="message" contenteditable=false>${username}: <span class="body">${input}</span>
...
function createTemplateAi(id, username, input) {
  $('.all-messages').append(
    `<li data=${username} id=${id}>
      <p class="message-ai" contenteditable=false>${username}: <span>${input}</span></p>
    </li>`
  );
}
...
$('.message').on('blur', function() {
  ...
  var targetId = parseInt(this.closest('li').id);
  var message = $(this).children('.body').text();
  for (i = 0; i <chatroom.messages.length; i++) {
    if (parseInt(chatroom.messages[i].id) === targetId) {
