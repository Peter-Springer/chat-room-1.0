...
function renderMessages() {
  $('li').remove();
  if (chatroom.messages.length <= 10) return renderAllMessages();
  renderTenMessages();
}
...
function generateResponse(input) {
  ...
  if (type === '?') {
    return responses.question[Math.ceil(Math.random()*responses.question.length)];
  } else {
    return responses.statement[Math.ceil(Math.random()*responses.statement.length)];
  }
}
